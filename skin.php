...
// File and new size
$filename = 'http://www.minecraft.net/skin/' . $user . '.png';

// Content type
header('Content-Type: image/png');

// Load
$rendered = imagecreatetruecolor(60, 120);
$source = imagecreatefrompng($filename);
if(!$source){
	$source = imagecreatefrompng("http://s3.amazonaws.com/MinecraftSkins/char.png");
}
$b = 30;
$s = 8;
...
else
{
	...
	// Copy the left leg
	imagecopyresampled($rendered, $source, $b * 1, $b * 2.5, $s / 2, $s * 2.5, $b / 2, $b * 1.5, $s / 2, $s * 1.5);

	// Copy the right leg
	imagecopyresampled($rendered, $fsource, $b / 2, $b * 2.5, $s * 7, $s * 2.5, $b / 2, $b * 1.5, $s / 2, $s * 1.5);
}
